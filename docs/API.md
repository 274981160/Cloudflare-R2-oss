...
### 分段 PUT（`Content-Range`）会被拒绝

标准 WebDAV 客户端上传大文件时可能发**分段 PUT**（Windows 资源管理器、macOS Finder
挂载时就是这么做的）：每次只发一段，靠 `Content-Range` 指定偏移，期望服务端写到指定位置。

R2 **不支持随机写入**，只能整对象覆盖。如果照单全收，客户端发第 1 段我们写成整个文件、
发第 2 段又把第 1 段覆盖掉……最后文件只剩最后一段，而客户端全程收到 `201` ——
属于**静默的数据损坏**。

所以本服务对分段 PUT 返回 **`501 Not Implemented`** 并给出说明；唯一的例外是
`Content-Range: bytes 0-(size-1)/size` 且长度一致（等价于一次性完整上传），照常处理。

> 也就是说：**断点续传只对网页端有效**。分片接口是 S3 风格的自定义扩展，
> 标准 WebDAV 客户端不会用它；超大文件请用网页端上传。
