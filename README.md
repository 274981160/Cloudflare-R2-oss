...
| 在线压缩/解压 | 选中文件/文件夹右键「压缩为 zip」直接存回网盘；zip 右键「在线解压」（内置 DEFLATE 解压器，不依赖第三方）；**解压遇到同名文件默认跳过、绝不静默覆盖**，会先问你要不要覆盖 |
...
| 流式预览 | 图片 / 视频 / 音频 / PDF 走**短时效签名直链**由浏览器原生加载：不用等整包下载完就能看，视频还能直接拖进度（服务端支持 Range/206）。签名拿不到时自动回退到整包取回 |
...
仓库自带一套 WebDAV / API 冒烟测试（237 项断言，覆盖全部 WebDAV 方法、锁、Range、权限、API Key、旧格式兼容、zip 打包完整性、在线解压/压缩与重名保护、回收站、覆盖保护、预览直链等），本地起好服务后直接跑：
...
│
├── scripts/
│   ├── smoke-test.sh    # WebDAV / API 冒烟测试（公开读模式），237 项断言
