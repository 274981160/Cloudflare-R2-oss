...
| 批量操作 | 多选文件与目录，批量移动、复制、删除、下载；**桌面端可直接拖拽**：把文件/文件夹拖到目标文件夹上即移动，选中多个可整批拖，也能拖到面包屑移到上级（拖到自己、自己的子目录或原地会被拒绝） |
...
网页端与后端之间的完整契约见 [`docs/API.md`](docs/API.md)，改接口时必须同步更新该文件。
WebDAV 客户端连接方式、兼容性、行为细节与排障见 [`docs/WEBDAV.md`](docs/WEBDAV.md)。常用入口：
