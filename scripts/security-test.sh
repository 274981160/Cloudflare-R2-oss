...
U1MK="$(u1code -X MKCOL "$BASE/webdav/user1")"
check "受限账号可建自己的目录（201，已存在则 405）" 1 "$([ "$U1MK" = "201" ] || [ "$U1MK" = "405" ] && echo 1 || echo 0)"
check "受限账号写自己的文件 201" 201 "$(u1code -X PUT --data 'mine' "$BASE/webdav/user1/mine.txt")"
check "受限账号的 token 能读自己的文件" 200 "$(code "$BASE/raw/user1/mine.txt?pt=$PT1")"
check "清理受限账号的测试文件 204" 204 "$(u1code -X DELETE "$BASE/webdav/user1/mine.txt")"
