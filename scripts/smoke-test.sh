...
section "12. 内部保留目录保护"
check "根目录 PROPFIND 不含内部目录" 0 "$(curl -s -X PROPFIND -H 'Depth: 1' "$BASE/webdav/" | grep -cF '_$flaredrive$/')"
...
check "api/list 不含内部目录" 0 "$(curl -s "$BASE/api/list/" | grep -cF '_$flaredrive$/')"
