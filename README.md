...
| 分享链接 | 点「复制分享链接」生成 `/s/{token}` 只读链接；页面可预览/下载，**没有任何写入口**，随时可吊销 |
| 编辑内撤销 | 编辑器里 `Ctrl+Z` / `Ctrl+Y`（或工具栏按钮）撤销重做上一次修改，连续输入合并成一步 |
| 历史版本 | 每次保存自动留一份旧内容（默认最多 10 个），可查看、恢复、删除 |
...
| `WEBDAV_VERSIONING` | 否 | `1` | 是否启用编辑历史（`0` 关闭，历史接口返回空并禁止恢复） |
| `WEBDAV_VERSION_LIMIT` | 否 | `10` | 每个对象保留的历史版本数量 |
| `WEBDAV_VERSION_MAX_SIZE` | 否 | `10485760` | 超过这个大小的对象不做历史快照，避免吃光存储 |
| `WEBDAV_VERSION_ALL_PUTS` | 否 | `0` | `1` 时对所有覆盖写都做快照（默认只对编辑器保存生效） |
...
3. 分享页是只读的：只有预览与下载，没有任何上传/修改/删除入口。
4. **`GUEST` 目录是匿名可写的，匿名也能删除里面的文件**（无法区分访客身份）。不要把重要文件放进 `GUEST` 指定的目录。
5. 缩略图默认不公开：网页端用带认证的请求取回再转 `blob:` URL 显示，不会把内部目录路径暴露给无认证请求。
6. `_$flaredrive$/` 内部目录（锁、API Key、分享记录、历史版本）**任何身份都不能通过 HTTP 直接读取**。
...
| 方式 | 作用范围 | 怎么用 |
| --- | --- | --- |
| 编辑内撤销 / 重做 | 当前这一次编辑会话里的改动 | 工具栏「撤销 / 重做」，或 `Ctrl+Z` / `Ctrl+Y`（macOS 用 `⌘Z` / `⌘⇧Z`） |
| 历史版本 | 已经保存到服务端的旧内容 | 工具栏「历史版本」→ 载入到编辑器 / 恢复此版本 / 删除此版本 |

- 编辑器保存时会带 `fd-snapshot: 1`，服务端在覆盖前把**旧内容**存进 `_$flaredrive$/versions/`，
  所以「保存之后才发现改错了」也能救回来。
- 恢复某个历史版本时，服务端会先把**当前内容**也存一份快照，因此恢复本身也能再回退。
- 每个对象默认保留最近 `WEBDAV_VERSION_LIMIT`（10）个版本；超过 `WEBDAV_VERSION_MAX_SIZE`
  （默认 10MB）的对象不做快照，避免把存储吃光。
- 历史版本接口都需要认证，并按该对象的读写权限判定；内部目录不可通过 HTTP 直接读取。
...
| `/api/versions/list/{key}` | `GET` | 列出某对象的历史版本 |
| `/api/versions/content/{key}/{id}` | `GET` | 读取某个历史版本的内容 |
| `/api/versions/restore/{key}/{id}` | `POST` | 恢复某个历史版本 |
| `/api/versions/remove/{key}/{id}` | `DELETE` | 删除某个历史版本 |
...
│   ├── TextEditor.vue  # 在线文本编辑器：语法着色、编辑内搜索、JSON 校验、撤销重做、历史版本
