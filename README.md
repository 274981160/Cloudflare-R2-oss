...
仓库自带一套 WebDAV / API 冒烟测试（242 项断言，覆盖全部 WebDAV 方法、锁、Range、权限、API Key、旧格式兼容、zip 打包完整性、在线解压/压缩与重名保护、回收站、覆盖保护、预览直链与缓存等），本地起好服务后直接跑：
...
│
├── scripts/
│   ├── smoke-test.sh    # WebDAV / API 冒烟测试（公开读模式），242 项断言
