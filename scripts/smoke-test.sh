...
section "13.x 分享访问密码（B2）"
SP="$W/share-pw.txt"
check "建分享密码测试文件 201" 201 "$(acode -X PUT --data 'top secret' "$SP")"
SPID="$(curl -s -u "$ADMIN" -X POST -H 'Content-Type: application/json' -d '{"key":"_smoke/share-pw.txt","password":"pw123"}' "$BASE/api/shares" | python3 -c "import json,sys;print(json.load(sys.stdin).get('token',''))" 2>/dev/null)"
check "带密码创建分享 201" 1 "$(printf '%s' "$SPID" | grep -c .)"
check "列表显示 hasPassword" 1 "$(curl -s -u "$ADMIN" "$BASE/api/shares" | python3 -c "import json,sys;items=json.load(sys.stdin)['shares'];print(1 if any(i.get('hasPassword') for i in items if i['key']=='_smoke/share-pw.txt') else 0)" 2>/dev/null)"
check "匿名未验证是密码页 200" 200 "$(code "$BASE/s/$SPID")"
check "密码页含提示" 1 "$(curl -s "$BASE/s/$SPID" | grep -c '需要访问密码')"
check "错误密码 401" 401 "$(acode -X POST -F 'password=wrong' "$BASE/s/$SPID")"
PWC="$(curl -s -i -X POST -F 'password=pw123' "$BASE/s/$SPID" | grep -i '^set-cookie' | sed 's/^[Ss]et-[Cc]ookie: //' | tr -d '\r' | cut -d';' -f1)"
check "正确密码签发凭证" 1 "$(printf '%s' "$PWC" | grep -c .)"
check "凭证访问拿到内容" "top secret" "$(curl -s -H "Cookie: $PWC" "$BASE/s/$SPID")"
check "清除密码 200" 200 "$(acode -X POST -H 'Content-Type: application/json' -d '{"key":"_smoke/share-pw.txt","password":""}' "$BASE/api/shares")"
check "清除后匿名直接访问 200" 200 "$(code "$BASE/s/$SPID")"
check "匿名拿到内容" "top secret" "$(curl -s "$BASE/s/$SPID")"
check "暂停分享 200" 200 "$(acode -X POST -H 'Content-Type: application/json' -d '{"key":"_smoke/share-pw.txt","suspended":true}' "$BASE/api/shares")"
check "暂停后访问 404" 404 "$(code "$BASE/s/$SPID")"
check "暂停页含提示" 1 "$(curl -s "$BASE/s/$SPID" | grep -c '分享者暂时关闭')"
check "恢复分享 200" 200 "$(acode -X POST -H 'Content-Type: application/json' -d '{"key":"_smoke/share-pw.txt","suspended":false}' "$BASE/api/shares")"
check "恢复后可访问 200" 200 "$(code "$BASE/s/$SPID")"
check "吊销测试分享 204" 204 "$(acode -X DELETE "$BASE/api/shares/$SPID")"
check "清理密码测试文件 204" 204 "$(acode -X DELETE "$SP")"
