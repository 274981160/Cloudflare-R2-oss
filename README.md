...
| 缩略图 | **按需加载**：只拉进入视口的行，照片多的目录在手机上不再一次性请求上百张 |
...
仓库自带一套 WebDAV / API 冒烟测试（191 项断言，覆盖全部 WebDAV 方法、锁、Range、权限、API Key、旧格式兼容、zip 打包完整性、在线解压/压缩与重名保护等），本地起好服务后直接跑：
...
│
├── scripts/
│   ├── smoke-test.sh    # WebDAV / API 冒烟测试（公开读模式），191 项断言
