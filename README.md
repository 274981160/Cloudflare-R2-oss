...
| 交互习惯 | **按设备自适应**：触屏单击即打开、长按出菜单；桌面单击选中、双击打开；支持 Ctrl/Cmd 加选、Shift 范围选择、方向键+回车键盘导航；可一键切成多选模式批量操作（⋮ 菜单或长按菜单进入） |
| 视图与主题 | 列表 / 卡片网格两种视图一键切换；主题跟随系统或手动浅色 / 深色；视图、主题、单击行为、排序都记在本机 |
...
交互与界面用 Playwright 做真实浏览器验证（桌面 + 触屏两种指针环境）：

| 场景 | 断言数 |
| --- | --- |
| 桌面：单击选中 / 双击打开 / Ctrl 加选 / Shift 范围 / 键盘导航 / 全选 / 视图与主题与单击行为的切换与持久化 / 多选模式 / 右键菜单 | 30 |
| 触屏（Pixel 5 模拟 + 真实触摸事件 + CDP 长按）：默认单击打开、轻触进入文件夹、长按弹菜单且不误开、多选模式、长按菜单里的多选入口 | 14 |
| 编辑器（整理缩进 / 替换 / Tab 与回车缩进 / 撤销 / JSONC） | 36 |
...
│   ├── FolderIcon.vue  # 自绘内联 SVG 文件夹图标（跟随主题色，不依赖 CDN）
...
│   ├── main.css        # 样式与设计变量（浅色/深色、列表/网格、图标配色）
